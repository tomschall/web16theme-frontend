'use strict';

var data = {
    '@id': 'https://www.fhnw.ch/de/searchbar.json?q=FHNW&category=web&template=false',
    'batching': {
        '@id': 'https://www.fhnw.ch/de/searchbar.json?q=FHNW&category=web&template=false',
        'first': 'https://www.fhnw.ch/de/searchbar.json?q=FHNW&category=web&b_start=0&template=false',
        'last': 'https://www.fhnw.ch/de/searchbar.json?q=FHNW&category=web&b_start=870&template=false',
        'next': 'https://www.fhnw.ch/de/searchbar.json?q=FHNW&category=web&b_start=5&template=false'
    },
    'category': 'web',
    'categoryTitle': 'Inhalte dieser Seite',
    'categoryUrl': 'https://www.fhnw.ch/search_web#q=FHNW',
    'categoryUrlText': 'Mehr anzeigen (873)',
    'items': [
        {
            '@id': 'https://www.fhnw.ch/de/startseite',
            '@type': 'Document',
            'Description': 'Willkommen an der Fachhochschule Nordwestschweiz FHNW. Die FHNW ist eine der führenden Fachhochschulen in der Schweiz und' +
                    ' ist mit ihren neun Hochschulen in der Lehre, Forschung, Weiterbildung und Dienstleistung tätig – innovativ und praxisorientiert.' +
                    ' Ihr breites Angebot an Studiengängen, ihre Nähe zur Praxis, ihre anwendungsorientierte und innovationsstarke Forschung sowie ihre' +
                    ' weltweite Vernetzung machen die FHNW zu einer attraktiven und vielfältigen Bildungsstätte, zu einer nachgefragten Praxispartnerin' +
                    ' und zu einer attraktiven Arbeitgeberin in der Nordwestschweiz.',
            'Title': 'FHNW',
            'description': 'Willkommen an der Fachhochschule Nordwestschweiz FHNW. Die FHNW ist eine der führenden Fachhochschulen in der Schweiz und ist mit' +
                    ' ihren neun Hochschulen in der Lehre, Forschung, Weiterbildung und Dienstleistung tätig – innovativ und praxisorientiert. Ihr breites Angebot' +
                    ' an Studiengängen, ihre Nähe zur Praxis, ihre anwendungsorientierte und innovationsstarke Forschung sowie ihre weltweite Vernetzung machen die' +
                    ' FHNW zu einer attraktiven und vielfältigen Bildungsstätte, zu einer nachgefragten Praxispartnerin und zu einer attraktiven Arbeitgeberin in der' +
                    ' Nordwestschweiz.',
            'path_string': '/Plone/de/startseite',
            'portal_type': 'Document',
            'review_state': null,
            'study_type': null,
            'taxonomy_competence': null,
            'taxonomy_subjectarea': [
                '1006'
            ],
            'title': 'FHNW',
            'UID': 'ed935fd8900f49538edeaac497f283da',
        },
        {
            '@id': 'https://www.fhnw.ch/de/die-fhnw/emagazin',
            '@type': 'Document',
            'Description': 'Einblicke in spannende Projekte der FHNW',
            'Title': 'FHNW eMagazin',
            'description': 'Einblicke in spannende Projekte der FHNW',
            'path_string': '/Plone/de/die-fhnw/emagazin',
            'portal_type': 'Document',
            'review_state': null,
            'study_type': null,
            'taxonomy_competence': null,
            'taxonomy_subjectarea': null,
            'title': 'FHNW eMagazin',
            'UID': '50926e65bb204c3b989158228f4746a3',
        },
        {
            '@id': 'https://www.fhnw.ch/de/die-fhnw',
            '@type': 'Document',
            'Description': 'Die Fachhochschule Nordwestschweiz FHNW: Zukunft bilden',
            'Title': 'Die FHNW',
            'description': 'Die Fachhochschule Nordwestschweiz FHNW: Zukunft bilden',
            'path_string': '/Plone/de/die-fhnw',
            'portal_type': 'Document',
            'review_state': null,
            'study_type': null,
            'taxonomy_competence': null,
            'taxonomy_subjectarea': null,
            'title': 'Die FHNW',
            'UID': 'acbdeda2dbd64779814ca25da395bc08',
        },
        {
            '@id': 'https://www.fhnw.ch/de/die-fhnw/alumni-fhnw',
            '@type': 'Document',
            'Description': 'Die Alumni bzw. die Absolventinnen und Absolventen der FHNW sind nach Hochschulen in selbstständigen Alumni-Vereinigungen organisiert.' +
                    ' Alumni FHNW bildet das Dach der Alumnivereine.',
            'Title': 'Alumni FHNW',
            'description': 'Die Alumni bzw. die Absolventinnen und Absolventen der FHNW sind nach Hochschulen in selbstständigen Alumni-Vereinigungen organisiert.' +
                    ' Alumni FHNW bildet das Dach der Alumnivereine.',
            'path_string': '/Plone/de/die-fhnw/alumni-fhnw',
            'portal_type': 'Document',
            'review_state': null,
            'study_type': null,
            'taxonomy_competence': null,
            'taxonomy_subjectarea': null,
            'title': 'Alumni FHNW',
            'UID':  'bc05c5ade57446179c447b21334d2cfa',
        },
        {
            '@id': 'https://www.fhnw.ch/de/die-fhnw/stiftung-fhnw',
            '@type': 'Document',
            'Description': 'Die Stiftung FHNW unterstützt die Fachhochschule Nordwestschweiz (FHNW) bei der Wahrnehmung ihrer gesellschaftlichen, kulturellen und' +
                    ' wissenschaftlichen Verantwortung. Sie fördert innovative, herausragende und kreative Projekte und Aktivitäten in der Lehre und' +
                    ' anwendungsorientierten Forschung.',
            'Title': 'Stiftung FHNW',
            'description': 'Die Stiftung FHNW unterstützt die Fachhochschule Nordwestschweiz (FHNW) bei der Wahrnehmung ihrer gesellschaftlichen, kulturellen und' +
                    ' wissenschaftlichen Verantwortung. Sie fördert innovative, herausragende und kreative Projekte und Aktivitäten in der Lehre und' +
                    ' anwendungsorientierten Forschung.',
            'path_string': '/Plone/de/die-fhnw/stiftung-fhnw',
            'portal_type': 'Document',
            'review_state': null,
            'study_type': null,
            'taxonomy_competence': null,
            'taxonomy_subjectarea': null,
            'title': 'Stiftung FHNW',
            'UID': 'dbb2278cd7044c54bf0ca098d59d1290',
        }
    ],
    'items_total': 873,
    'template': 'false',
    'highlighting':{
        'ed935fd8900f49538edeaac497f283da': {
           'Title':['<em>FHNW</em>']
        },
        '50926e65bb204c3b989158228f4746a3': {
            'Title':['<em>FHNW</em>'],
            'Description':['Welcome to <em>FHNW</em> University of Applied Sciences and Arts Northwestern Switzerland. <em>FHNW</em> University']
        },
        'acbdeda2dbd64779814ca25da395bc08':{
            'Title':['<em>FHNW</em>'],
            'Description':['Willkommen an der Fachhochschule Nordwestschweiz <em>FHNW</em>. Die <em>FHNW</em> ist eine der führenden']
        },
        'bc05c5ade57446179c447b21334d2cfa':{
            'Title':['Imagebroschüre <em>FHNW</em>']
        },
        'dbb2278cd7044c54bf0ca098d59d1290':{
            'Title':['La <em>FHNW</em>']
        }
    }
};

module.exports = data;
